...
t2_rank <- t2 %>% mutate(highest_rank_increase=rank-lag(rank)) %>% 
  filter(week!=1) %>%
  group_by(artist, track, date.entered) %>% summarize(highest_rank_increase=-min(highest_rank_increase, na.rm=T))
...
t3 <- t2 %>% separate(col=date.entered, into=c("year", "month", "date"))
...
t4 <- t3 %>% group_by(artist, track, month) %>% summarise(highest_rank=min(rank, na.rm=T))
t4
...
t5 <- billboard %>%
  filter(!is.na(wk1)) %>%
  arrange(wk1) %>%
  select(artist, track, date.entered, wk1_rank=wk1)
...
t6 <- t1 %>% 
  filter(!startsWith(as.character(date.entered), "1999")) %>%
  group_by(artist) %>%
