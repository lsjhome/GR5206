...
length_sex_ring <- abalone %>% filter(.$length > 0.6) %>% arrange(sex, desc(rings)) %>% select(sex, diameter, height, rings)
...
count_prop <- abalone %>% count(sex)
count_prop$prop <- count_prop$n / sum(count_prop$n)
names(count_prop)[2] <- "count"
count_prop
...
mean_max_min <- abalone %>% group_by(sex) %>% summarize(mean(shucked_weight), max(shucked_weight), min(shucked_weight))
names(mean_max_min)[2:4] <- c("weight_mean", "weight_max","weight_min")
mean_max_min
...
filter_na_rename_select_everything <- abalone %>% filter(is.na(diameter) | diameter > 0.36) %>%
  rename(index=X) %>% select(index, sex, length, diameter, rings, everything())
...
transmute <- abalone %>% transmute(whole_weight_in_mg = whole_weight * 1000, 
...
first_1000_rank <- abalone %>% select (diameter, rings) %>% arrange(rings) %>%mutate(rings_rank=row_number()) %>% 
  filter(rings_rank <=1000) %>% arrange(rings_rank)
