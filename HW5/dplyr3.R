...
airquality_replace_na$Ozone <- airquality_replace_na$Ozone %>% replace_na(mean(., na.rm=T))
...
airquality_remove_na <- drop_na(airquality_replace_na, Solar.R)
...
airquality_wind <- airquality_filtered %>% mutate(Wind=case_when(Wind >=1 & Wind < 9 ~ 7.4,
                                                                 Wind >=9 & Wind <= 20 ~ 11.5,
                                                                 TRUE ~ 21))
