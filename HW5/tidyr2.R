...
library(dplyr)
library(tibble)
activities <- as_tibble(read_csv('./data/activities.csv'))
...
act_gathered <- activities %>% 
  gather(key="var", value="score", c(-id, -trt)) %>%
  arrange(id, trt)
...
act_separated <- act_gathered %>% 
  separate(col = var, into = c("action", "time"), sep="[.]")
...
col_ix <- which(is.na(act_spread), arr.ind = TRUE)[1, 'col']
