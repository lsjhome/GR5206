...
airbnb <- as_tibble(read_csv("data/Airbnb_NYC_2019.csv"))
...
nyc_price <- airbnb %>% 
  ...
  summarise(avg_price=mean(price, rm.na=T)) %>%
...
by_type <- airbnb %>% 
  ...
  group_by(room_type) %>% 
  summarise(price=median(price), availability_365=median(availability_365)) 
...
neighbourhood_room_wider <- neighbourhood_room %>%
  select(neighbourhood_group, room_type, prop) %>%
