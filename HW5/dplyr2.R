...
longevity <- nations %>% filter(year==2016 & !is.na(life_expect) & !is.na(gdp_percap)) %>% 
...
ea_na_75_85 <- longevity %>% filter(region=="East Asia & Pacific" | region=="North America") %>%
  filter(life_expect > 75 & life_expect < 85) %>% arrange(desc(life_expect))
...
top_10_perc_us <- longevity %>% mutate(perc_rank=percent_rank(life_expect)) %>% 
  arrange(desc(perc_rank)) %>% filter(perc_rank>=0.9 | (country=='United States' & perc_rank<0.9))
...
gdp_by_region<-nations %>%
  mutate(total_gdp=gdp_percap*population) %>%
  group_by(region,year) %>%
  summarise(total_gdp=sum(total_gdp,na.rm=T)/1000000000000)
...
p_countries <- nations %>% filter(year==2016) %>% group_by(income) %>% summarise(p=sum(life_expect>70, na.rm=T)/n())