...
t1 <- useR2016 %>% select(Q2:Q13_F, Q25, -Q12)
...
t1_longer <- t1 %>% 
  pivot_longer(Q13:Q13_F, names_to = 'cases', values_to = 'comments', values_drop_na = T)
...
t1_longer_rename <- t1_longer %>% 
  rename(sex=Q2, age=Q3, degree=Q7, academic_status=Q8, experience=Q11, value=Q25)
...
separate_drop_redundant <- t1_longer_rename %>% 
  separate(cases, into=c("case_name", "category"), sep = "_", fill="right") %>% 
...
filter_na_unite <- separate_drop_redundant %>% 
  ...
  unite(col=age_and_experience, age, experience, sep=' && ')
