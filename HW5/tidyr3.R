...
t1 <- warpbreaks %>% 
  ...
  filter(!(breaks > 100 | breaks < 0)) %>%
  filter(wool %in% c("A","B") & tension %in% c("L","H","M")) %>%
...
t2 <- t1 %>% 
  group_by(wool, tension) %>% count(name="n")
...
t3 <- t1 %>% 
  group_by(tension) %>% 
  pivot_wider(names_from = tension, values_from = breaks, values_fn = list(breaks=sum))
