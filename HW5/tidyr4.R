...
library(readr)
library(dplyr)
mobility <- as_tibble(read_csv("data/mobility.csv"))
...
stats_by_state <- mobility %>%
  group_by(State) %>%
  summarize_at(.vars = vars(Commute, Mobility) ,
               .funs = funs(mean,sd))
...
stats_by_state_longer <- stats_by_state %>%
  pivot_longer(cols = Commute_mean:Mobility_sd, names_to = 'Stat' , values_to = 'Value') %>%
  separate(col=Stat, into= c("Variable", "Stat"), sep="_")
...
stats_by_state_ranked <- stats_by_state_longer %>%
  ...
  select(-Stat) %>%
  group_by(Variable) %>%
  mutate(percentage_rank = percent_rank(Value)) %>%
  select(-Value) %>%
  pivot_wider(names_from = Variable, values_from = percentage_rank) %>%
...
coordinates <- mobility %>%
  group_by(State) %>%
  summarize( latitude = round(mean(Latitude),3) , 
             longitude = round(mean(Longitude),2)) %>%
  unite(col = Coordinates, latitude,longitude, sep = "_")
