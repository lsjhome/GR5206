...
nick_name_list_update <- 
  map_chr(nick_name_list, function(x) 
    paste(x, collapse=" or ")) 
...
sentences_list <- map2_chr(.x = name_list, .y = nick_name_list_update, .f = function(.x, .y){
  return (paste(.x, "can be called",.y))
})
