#' HW3: map5
#'
#' In this exercise, we will play around with `got_chars` data from `repurrrsive` package. 
#' For this exercise, you are not allowed to use `for`, `while`, or `repeat` loops.
#'
#' 1. Load the `purrr` and `repurrrsive` packages. 
#' Use `map_chr` to extract all the `name` from `got_chars`, store them into a vector of characters called `name_list`.
...
name_list <- map_chr(got_chars, function(x) x$name)

nick_name_list<-map(got_chars, 
                    function(x) {
                      if(length(x$aliases)>0) 
                        return(x$aliases)
                      else return (NULL)})
  

#' 2. Make a function called `hello` with argument `x` (e.g., `hello(x)`). 
#' Inside the `hello` function, use `paste` to concatenate `Hello `, `x` and `!\n`, the `sep` should be empty.
