...
set.seed(5206)
seed <- .Random.seed
l1 <- map(1:5, function(x) sample(1:10, 20, replace = T))
...
#'    Note that, when using `data.frame`, the first column will be automatically converted as a factor 
#'    (i.e., do NOT use `stringsAsFactors = FALSE`).
...
#' 4. Similarly as in 3, create a data frame called `df2` with first column `A` and values `letters[1:5]`, 
#' second column `D` with values `3:7`, third column `E` with values `4:8`.
#'    Create yet another data frame called `df3` with first column `A` and values `letters[1:5]`,
#'     second column `G` with values `5:9`, third column `H` with values `6:10`.
