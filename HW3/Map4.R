...
mtcars_factor <- mtcars_factor %>% modify_at(c('cyl', 'gear'), as.factor)
...
mtcars_numeric_sd <- mtcars_factor %>% map_if(.p=is.numeric, .f=~sd(.x))
mtcars_numeric_sd <- map_if(mtcars_factor, .p=is.numeric, .f=~sd(.x))
...
mtcars_factor_level <- map_if(mtcars_factor, .p=is.factor, .f=~length(unique(levels(.x))))