...
#'
#' In this exercise, we will write a function computing the k^{th} order statistics 
#' and a function computing the density of a mixture model.
#' Order Statistics Reference: https://en.wikipedia.org/wiki/Order_statistic.
#'
#' 1. Implement a function `kth_order(vec, k)` that takes as input a vector and returns its k^{th} order statistics. 
#' If `k` is not an index of the vector, the function should return `NA`.
...
kth_order(c(3,4,6,5,7,1), 5)


#' 2. Implement a function `order_factory(k)` that takes as input an index `k` and 
#' returns a function that compute the k^{th} order statistics of a vector `vec` using `kth_order`. 
...
my_mixture <- function(x, c1, mu1, sigma1, c2, mu2, sigma2) c1*dnorm(x, mu1, sigma1)+c2*dnorm(x, mu2, sigma2)
