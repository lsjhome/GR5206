...
is_ok <- !map_lgl(transpose(y)$result, is_null)
...
invalid_column <- names(is_ok)[!is_ok]
m2 <- flatten_dbl(transpose(y[is_ok])$result)
...
m3 <- map_dbl(iris, possibly(max, NA))
...
s <- map_dbl(iris, possibly(mean, NA))
