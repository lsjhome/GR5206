...
# 4. Use `strsplit` function to split the string "statistics" into characters and 
#    then apply the `move` function to move each element in the string by 5 steps using `sapply`. The result should be stored into a vector `v1`.
...
{
  cur_idx <- t1$position[t1$character == char]
  next_idx <- ifelse((cur_idx + nstep) %% 26==0, 26, (cur_idx + nstep) %% 26)
  next_val <- t1$character[t1$position == next_idx]
